...
constexpr float KNOCKBACK_CONST    = 2.5f;   // 넉백 수평 세기(이동 반대 방향).
constexpr float KNOCKBACK_MAX      = 1000;   // 넉백 중 속도 상한.
constexpr int   KNOCKBACK_DURATION = 4;      // 넉백 윈도우 길이(프레임).

constexpr float PLAYER_SPEED          = 0.335f;  // 휠 한 칸이 더하는 표시 속도.
...
constexpr int CRIT_SHAKE_INTENSITY = 10;
...
// 총알로 적 처치 시 비차단 카메라 진동: 세기 / 지속 프레임(짧은 펀치감).
constexpr float BULLET_KILL_SHAKE_INTENSITY = 6.0f;
constexpr int   BULLET_KILL_SHAKE_FRAMES = 6;
